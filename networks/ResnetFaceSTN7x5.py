...
class ResnetFaceSTN7x5(Module):
    def __init__(self, num_layers=50, drop_ratio=0.4, mode='ir_se'):
        super(ResnetFaceSTN7x5, self).__init__()
        ...
        self.fcloc[1].weight.data.zero_()
        self.fcloc[1].bias.data.copy_(torch.tensor([.71, 0, 0,
                                                    0, 1, 0],
                                                   dtype=torch.float32))
    ...
    def stn(self, x):
        ...
        grid = F.affine_grid(theta, torch.Size([N, 3, 112, 80]),
                             align_corners=False)
