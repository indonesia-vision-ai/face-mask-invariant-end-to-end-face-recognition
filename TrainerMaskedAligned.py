...
from datasets.eval.lfw import LFWMaskedAlign112
...
class FRTrainer():
    ...
    def __init__(self, param_config, net_config, dataset_config,
                 optim_params_func=None, scheduler=None,
                 scheduler_params=None, eval_transform=None):

        if (scheduler is not None) != (scheduler_params is not None):
            raise Exception("'scheduler' and 'scheduler_params' are mutually inclusive!")
    ...
    def _evaluate(self):
        ...
        lfw_dataset = LFWMaskedAlign112('datasets/eval/data/lfw',
