...
class CPLFWAlign112(Dataset):
    def __init__(self, data_folder):
        self.path = data_folder

        self.pairs_file = pd.read_csv(
            os.path.join(self.path, "cplfw_pair_list.tsv"),
            names=['p1','p2','gt'], sep='\t').to_numpy()

        self.images = bcolz.carray(
            rootdir=os.path.join(self.path, "cplfw"), mode='r')
...
class CPLFWRaw(Dataset):
    r"""CPLFW dataset with original unmodified images
    ...
    def __init__(self, data_folder, transform=None):
        ...
        self.pairs_file = pd.read_csv(
            os.path.join(self.path, "cplfw_pairs.csv")).to_numpy()
...
class CPLFWMasked(CPLFWRaw):
    r"""CPLFW dataset with facemask augmented images
...
class CPLFWMask2Raw(Dataset):
    def __init__(self, data_folder, transform=None):
        self.path = data_folder
        
        raw_data_root = os.path.join(self.path, 'images/')
        mask_data_root = os.path.join(self.path, 'cplfw_masked/')

        self.pairs_file = pd.read_csv(os.path.join(self.path, "cplfw_pairs.csv"))
...
class CPLFWMaskedAligned(CPLFWRaw):
    r"""CPLFW dataset with facemask augmented images
    ...
    def __init__(self, data_folder, transform=None):
        ...
        self.data_root = os.path.join(
            self.path, 'cplfw_masked_aligned')
...
class CPLFWMask2RawAligned(Dataset):
    def __init__(self, data_folder, transform=None):
        self.path = data_folder
        
        raw_data_root = os.path.join(self.path, 'images_aligned/')
        mask_data_root = os.path.join(self.path, 'cplfw_masked_aligned/')
