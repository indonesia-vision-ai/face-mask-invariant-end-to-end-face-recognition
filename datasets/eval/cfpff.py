...
class CFPFFAlign112(Dataset):
    def __init__(self, data_folder):
        self.path = data_folder

        self.pairs_file = pd.read_csv(
            os.path.join(self.path, "cfp_ff_pair_list.tsv"),
            names=['p1','p2','gt'], sep='\t').to_numpy()

        self.images = bcolz.carray(
            rootdir=os.path.join(self.path, "cfp_ff"), mode='r')
...
class CFPFFRaw(Dataset):
    r"""CFPFF dataset with original unmodified images
    ...
    def __init__(self, data_folder, transform=None):
        ...
        self.images = glob.glob(os.path.join(
            self.data_root, '**', 'frontal', '*.jpg'))
        
        self.pairs_file = pd.read_csv(
            os.path.join(self.path, "Protocol", "cfp_ff_pairs.csv")).to_numpy()
...
class CFPFFMasked(CFPFFRaw):
    r"""CFPFF dataset with facemask augmented images
...
class CFPFFMask2Raw(Dataset):
    def __init__(self, data_folder, transform=None):
        ...
        self.pairs_file = pd.read_csv(os.path.join(self.path, "Protocol", "cfp_ff_pairs.csv"))
...
class CFPFFMaskedAligned(CFPFFRaw):
    r"""CFPFF dataset with facemask augmented images
...
class CFPFFMask2RawAligned(Dataset):
    def __init__(self, data_folder, transform=None):
        self.path = data_folder
